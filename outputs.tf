...
output "cloudwatch_log_group_name" {
  ...
  value       = local.logging.access_log_enabled ? aws_cloudwatch_log_group.api_gateway_access_logs[0].name : null
}

output "cloudwatch_log_group_arn" {
  description = "ARN of the CloudWatch log group for API Gateway access logs"
  value       = local.logging.access_log_enabled ? aws_cloudwatch_log_group.api_gateway_access_logs[0].arn : null
}
