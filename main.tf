...
resource "aws_cloudwatch_log_group" "api_gateway_access_logs" {
  count             = local.logging.access_log_enabled ? 1 : 0
  name              = "/aws/apigateway/${module.label.id}-access-logs"
  tags              = module.label.tags
  retention_in_days = local.logging.access_log_retention_in_days
}
...
resource "aws_api_gateway_stage" "this" {
  ...
  description   = local.stage.description

  cache_cluster_enabled = local.stage.cache_cluster_enabled
  cache_cluster_size    = local.stage.cache_cluster_enabled ? local.stage.cache_cluster_size : null
  xray_tracing_enabled  = local.stage.xray_tracing_enabled

  dynamic "access_log_settings" {
    for_each = local.logging.access_log_enabled ? [1] : []
    content {
      destination_arn = aws_cloudwatch_log_group.api_gateway_access_logs[0].arn
      format          = replace(local.logging.access_log_format, "\n", "")
    }
  }

  variables = local.stage.variables
...
resource "aws_cloudwatch_log_group" "api_gateway_execution_logs" {
  name              = "API-Gateway-Execution-Logs_${aws_api_gateway_rest_api.this.id}/${local.stage.name}"
  retention_in_days = local.logging.execution_log_retention_in_days
  tags              = module.label.tags
}
